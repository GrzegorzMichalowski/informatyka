...
function createSteps(values, x) {
  ...
  for (let i = 0; i < values.length; i += 1) {
    out.push({ type: "compare", i, val: values[i], x });
    if (values[i] === x) {
      out.push({ type: "found", i, val: values[i], x });
      break;
    }
  }
  out.push({ type: "done" });
...
function render() {
  ...
  arr.forEach((val, idx) => {
    ...
    if (idx === currentIndex) cell.classList.add("mid");
...
function describeStep(step) {
  switch (step.type) {
    case "compare":
      return `Sprawdzam a[${step.i}] = ${step.val}.`;
    case "found":
      return `Znaleziono! Wartość ${step.x} na pozycji ${step.i}.`;
    case "done":
